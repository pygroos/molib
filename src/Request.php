...
class Request
{
    ...
    public function get( $arrParam, & $arrResponse )
    {
        ...
        $this->http( $arrParam, $arrResponse, false );

        return $arrResponse;
    }

    public function post( $arrParam, & $arrResponse, $isJson = false )
    {
        if ( ! is_array( $arrParam ) || 0 == count( $arrParam ) )
        {
            return self::ERROR_PARAMETER;
        ...
        $this->http( $arrParam, $arrResponse, $isJson );

        return $arrResponse;
    }

    public function http( $arrParam, & $arrResponse, $isJson )
    {
        ...
        $nHttpCode = 0;
        $sResponse = '';
        $nRet = $this->_sendHttpRequest( $arrRequest, $nTimeout, $nHttpCode, $sResponse, $isJson );
    ...
    private function _sendHttpRequest( $arrRequest, $nTimeout, & $nHttpCode = 0, & $sResponseBody = null, $isJson )
    {
        ...
        if ( $this->_isValidCUrlHandle( $oCurl ) )
        {
            ...
            {
                if ($isJson) {
                    $requestParam = json_encode($arrRequest['data']);
                } else {
                    $requestParam = $arrRequest['data'];
                }
                curl_setopt( $oCurl, CURLOPT_POST, true );
                curl_setopt( $oCurl, CURLOPT_POSTFIELDS, $requestParam );
                curl_setopt( $oCurl, CURLOPT_URL, $arrRequest['url'] );
            }
            //
